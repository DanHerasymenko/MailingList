...
func GetEmailBatch(db *sql.DB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		quetyOptions := mdb.GetEmailBatchQueryParams{}
		fromJson(req.Body, &quetyOptions)

		if quetyOptions.Count <= 0 || quetyOptions.Page <= 0 {
		...
		returnJson(w, func() (interface{}, error) {
			...
			return mdb.GetEmailBatch(db, quetyOptions)
		})
...
func Serve(db *sql.DB, bind string) {
	...
	log.Printf("JSON API serverr listening on %v\n", bind)
